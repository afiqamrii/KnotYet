...
export const SecretNumberRaceGame: React.FC<Props> = ({ onEndGame }) => {
  ...
  return <section className="secret-race-card" aria-live="polite">
    <header className="secret-race-header"><div><span><Zap /> SECRET NUMBER RACE</span><h2>First to the answer wins.</h2></div><div className="secret-race-score"><strong>{myScore}</strong><small>YOU</small><i>:</i><strong>{theirScore}</strong><small>{partnerName}</small></div><button type="button" className="game-end-button" onClick={onEndGame}>End game</button></header>
